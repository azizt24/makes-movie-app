import { ORIGINAL_IMG, PROFILE_IMG, YOUTUBE } from '../config/constants.js';
const aggregateData = (tmdbData, omdbData, reviewsData) => {
  ...
  const combinedWriters = Array.from(
    new Set([
      ...tmdbWriters,
      ...(omdbData.Writer
        ? omdbData.Writer.split(', ').map(writer => writer.trim())
        : []),
    ])
  );

  const reviews =
    reviewsData && Array.isArray(reviewsData.results)
  ...
  return {
    poster: tmdbData.poster_path
      ? `${ORIGINAL_IMG}${tmdbData.poster_path}`
      : null,
    backdrop: tmdbData.backdrop_path
      ? `${ORIGINAL_IMG}${tmdbData.backdrop_path}`
      : null,
    ...
    trailers: tmdbData.videos
      ? tmdbData.videos.results.map(video => `${YOUTUBE}=${video.key}`)
      : [],
